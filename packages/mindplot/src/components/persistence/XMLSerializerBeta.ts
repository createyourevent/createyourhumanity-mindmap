...
class XMLSerializerBeta implements XMLMindmapSerializer {
  ...
  _topicToXML(document: Document, topic: NodeModel) {
    ...
    // HTML
    const htmls = topic.findFeatureByType('htmlForm');
    htmls.forEach((html) => {
        const htmlDom = this._htmlToXML(document, html);
        parentTopic.append(htmlDom);
    });
  ...
  _htmlToXML(document, html) {
    const htmlDom = document.createElement('htmlForm');
    htmlDom.setAttribute('id', html.getHtmlType());
    return htmlDom;
}
    ...
    for (let i = 0; i < children.length; i++) {
      ...
      if (child.nodeType === 1) {
        ...
        if (child.tagName === 'topic') {
          ...
          const html = this._deserializeHtml(child);
          topic.addFeature(html);
        }
      }
    }
  ...
  _deserializeHtml(domElem: Element) {
    console.log(domElem);
    return FeatureModelFactory.createModel('htmlForm', { id: domElem.getAttribute('id') });
  }
