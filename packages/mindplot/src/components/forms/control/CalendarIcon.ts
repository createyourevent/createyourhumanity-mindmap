...
class CalendarIcon extends Icon {
    ...
    constructor(topic: Topic, calendarModel: CalendarModel, readOnly: boolean, isProfile?: boolean) {
    ...
    _buildTooltipContent() {
      ...
      if(!this._isProfile) {
        ...
        const desc_value = $('<div class="col-md-6"></div>').text(this._calendarModel.getDescription())
        .css({
          'white-space': 'pre-wrap',
          'word-wrap': 'break-word',
        });
