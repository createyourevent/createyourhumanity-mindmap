import SelectImage from '../../../../assets/icons/html/control/selectbox.png';
...
class SelectIcon extends Icon {
    ...
    private _selectModel: ControlModel;
    ...
    constructor(topic: Topic, textfieldModel: SelectModel, readOnly: boolean, isProfile?: boolean) {
        ...
        super(SelectIcon.IMAGE_URL);
        this._selectModel = textfieldModel;
        this._topic = topic;
    ...
    _buildTooltipContent() {
      ...
      if(!this._isProfile) {
        if ($('body').find('#popoverProperties').length === 1) {
          ...
          text.text(this._selectModel.getKey());
          return text;
        }
        ...
        const req_value = $('<div class="col-md-6"></div>').text(this._selectModel.getRequired())
        .css({
          'white-space': 'pre-wrap',
          'word-wrap': 'break-word',
        });
        ...
        const key_value = $('<div class="col-md-6"></div>').text(this._selectModel.getKey())
        .css({
          'white-space': 'pre-wrap',
          'word-wrap': 'break-word',
        });
        ...
        const desc_value = $('<div class="col-md-6"></div>').text(this._selectModel.getDescription())
        .css({
          'white-space': 'pre-wrap',
          'word-wrap': 'break-word',
        });
        ...
        let v = '';
        for(let i = 0; i < this._topic.getChildren().length; i++) {
          const t = this._topic.getChildren()[i];
          if(i === Number(val)-1) {
            v = t.getText();
          }
        }
        ...
        if((val === null && gra === null) || (val === null && gra !== null)) {
          ...
          textfield_val = $('<div class="col-md-12"></div>').text(v);
