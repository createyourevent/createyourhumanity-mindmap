...
class TextareaIcon extends Icon {
    ...
    constructor(topic: Topic, textfieldModel: TextareaModel, readOnly: boolean, isProfile?: boolean) {
    ...
    _buildTooltipContent() {
      ...
      if(!this._isProfile) {
        ...
        const desc_value = $('<div class="col-md-6"></div>').text(this._textareaModel.getDescription())
        .css({
          'white-space': 'pre-wrap',
          'word-wrap': 'break-word',
        });
