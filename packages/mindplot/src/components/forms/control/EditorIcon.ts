...
class EditorIcon extends Icon {
    ...
    constructor(topic: Topic, editorModel: EditorModel, readOnly: boolean, isProfile?: boolean) {
    ...
    _buildTooltipContent() {
      ...
      if(!this._isProfile) {
        ...
        const desc_value = $('<div class="col-md-6"></div>').text(this._editorModel.getDescription())
        .css({
          'white-space': 'pre-wrap',
          'word-wrap': 'break-word',
        });
        ...
        if((val === null && gra === null) || (val === null && gra !== null)) {
          ...
          textfield_val = $('<div class="col-md-12"></div>').html(val);
