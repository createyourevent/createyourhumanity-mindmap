...
class CreateYourHumanityPersistenceManager extends PersistenceManager {
  ...
  saveMapXml(mapId: string, mapXml: Document, pref: string, saveHistory: boolean, events): void {
    const data = {
      id: mapId,
      text: new XMLSerializer().serializeToString(mapXml),
    }
...
export default CreateYourHumanityPersistenceManager;
